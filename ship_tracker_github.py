#!/usr/bin/env python3
"""
Ship tracker with Telegram alerts - GitHub Actions version
"""
...
def load_state():
    ...
    try:
        with open('ship_state.json') as f:
            return json.load(f)
    except:
        pass
...
def main():
    print("=" * 50)
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Ship Tracker Check")
    print("=" * 50)
    ...
    if in_geofence_now and not was_in_geofence:
        ...
        # Still in geofence - alert if very close
        approaching = is_approaching(current_distance, state['last_distance'])
    ...
    print(f"Status: {'IN GEOFENCE' if in_geofence_now else 'Outside geofence'}")
    print("=" * 50)
